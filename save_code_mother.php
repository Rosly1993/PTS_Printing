...
$sql = "INSERT INTO `tbl_palletserial` (`model`, `cavity`, `lotserial`, `quantity`, `copies`,`date_created`) VALUES ";
$sql .= "('{$model}', '{$cavity}', '{$sublot_serial}', '{$quantity}', '{$copies}', '{$date}')";

//update main table

$update = $conn1->query("UPDATE `tbl_serial_printing` set `mother_lot_serial_generated` = '1' where mother_serial = '{$sublot_serial}' ");
...
if($conn1->query($sql)){
    ...
    echo "<script> location.replace('./?page=generated_codes_mother'); </script>";
} else {

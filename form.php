...
<?php
include('db_main.php');
$query_serial = $conn1->query("SELECT id,model,cavity,sublot_serial, sum(quantity) as quantity FROM `tbl_serial_printing` where  lot_serial_generated is null group by sublot_serial  ");
?>
 

<div class="card rounded-0 shadow">
    <div class="card-header">
        <h3><b>Generate Printable Sublot QRCodes</b></h3>
    </div>
    <div class="card-body">
        <div class="container">
        ...
        <tbody>
            <?php while($row = mysqli_fetch_array($query_serial)) { ?>  
                <tr>
                    ...
                    <td><input type="text" class="form-control form-control-sm w-100" name="qrcode" value="<?php echo $row['sublot_serial']; ?>" required readonly></td>
                    ...
                    <td class="px-1 py-1 text-center"><button style="border-radius: 5px; background-color:#007F73"><a href="save_code.php?id=<?php echo $row["id"]; ?>&&model=<?php echo $row["model"]; ?>&&cavity=<?php echo $row["cavity"]; ?>&&quantity=<?php echo $row["quantity"]; ?>&&sublot_serial=<?php echo $row["sublot_serial"]; ?>" style="text-decoration: none;color: white">Generate QR</a></button></td>
                </tr>
            <?php } ?>
        </tbody>
    </table>
